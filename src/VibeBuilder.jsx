...
const ToggleSortBtn = ({
    ...
    return (
        <div className={`flex-1 h-full relative overflow-visible ${roundedClass}`} style={{ transform: 'translateZ(0)', ...extremityStyle }}>
...
const FileFilterBtn = ({ fileFilter, setFileFilter, hideGlow = false, is3DMode = false }) => {
    ...
    // En 3D, extrémités rectangulaires avec coins 0.5rem au lieu de rounded-full
    const roundedClass = is3DMode ? '' : 'rounded-r-full';
    const extremityStyle = is3DMode ? { borderTopRightRadius: '0.5rem', borderBottomRightRadius: '0.5rem' } : {};
    ...
    return (
        <div className={`flex-1 h-full relative overflow-visible ${roundedClass}`} style={{ transform: 'translateZ(0)', ...extremityStyle }}>
            ...
            <button
                onClick={cycleFilter}
                className={`relative z-10 w-full h-full flex items-center justify-center text-white ${roundedClass}`}
                style={extremityStyle}
            >
            ...
            {vibingSong && (() => {
                ...
                >
                    ...
                    >
                        ...
                        {/* Curseur transparent draggable — wrapper fixe + contenu animé + masque statique */}
                        <div
                        ...
                        >
                            ...
                            {is3DMode && <CylinderMask is3DMode={true} intensity={(isAtLeftThreshold || isAtRightThreshold || (isAtCenter && previewHasDragged)) ? CONFIG.CAPSULE_CYLINDER_INTENSITY_ON : CONFIG.CAPSULE_CYLINDER_INTENSITY_OFF} className="pointer-events-none" />}
                        </div>
            ...
            >
                ...
                <div className="flex items-center gap-2 relative z-20">
                    ...
                    {cardSwipeDirection === 'horizontal' && !showDeleteConfirm && !dragState && !isSearching && searchOverlayAnim === 'none' && (() => {
                        ...
                        return (
                            <div
                                className={`absolute inset-0 ${is3DMode ? '' : 'rounded-full'} flex items-center justify-center z-40 overflow-hidden`}
                                style={{
                    ...
                    {dragState ? (
                        <>
                            ...
                            <div
                                className={`absolute inset-0 ${is3DMode ? '' : 'rounded-full'} animate-neon-glow`}
                                style={{
                                    borderRadius: is3DMode ? '0.5rem' : undefined,
                            ...
                            <div
                                className={`absolute inset-0 ${is3DMode ? '' : 'rounded-full'} flex items-center justify-center animate-pop border-2`}
                                style={{
                                    borderRadius: is3DMode ? '0.5rem' : undefined,
                            ...
                            >
                                <div className={`absolute inset-0 ${is3DMode ? '' : 'rounded-full'} overflow-hidden`} style={{ borderRadius: is3DMode ? '0.5rem' : undefined }}>
                                    <CylinderMask is3DMode={is3DMode} intensity={CONFIG.CAPSULE_CYLINDER_INTENSITY_ON} className={is3DMode ? '' : 'rounded-full'} />
                                </div>
                            ...
                            <div className="w-full h-full flex items-center gap-2 overflow-visible relative">
                                ...
                                {(isSearching || searchOverlayAnim !== 'none') && (
                                    ...
                                    >
                                        ...
                                        >
                                            <CylinderMask intensity={CONFIG.CAPSULE_CYLINDER_INTENSITY_OFF} className={is3DMode ? '' : 'rounded-l-full'} is3DMode={is3DMode} style={is3DMode ? { borderTopLeftRadius: '0.5rem', borderBottomLeftRadius: '0.5rem' } : {}} />
                                        ...
                                        <div className={`h-full relative overflow-visible ${is3DMode ? '' : 'rounded-r-full'}`} style={{ flex: CONFIG.CLOSE_BTN_FLEX, borderTopRightRadius: is3DMode ? '0.5rem' : undefined, borderBottomRightRadius: is3DMode ? '0.5rem' : undefined }}>
                                            ...
                                            />
                                            <CylinderMask intensity={CONFIG.CAPSULE_CYLINDER_INTENSITY_ON} className={is3DMode ? '' : 'rounded-r-full'} is3DMode={is3DMode} style={is3DMode ? { borderTopRightRadius: '0.5rem', borderBottomRightRadius: '0.5rem' } : {}} />
                                            <button
                                                onClick={() => {
                                                    if (searchOverlayAnim !== 'none') return;
