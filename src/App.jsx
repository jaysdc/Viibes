...
import { UNIFIED_CONFIG, FOOTER_CONTENT_HEIGHT_CSS, getPlayerHeaderHeightPx, getPlayerFooterHeightPx, getBeaconHeightPx, ALL_GRADIENTS, GRADIENT_NAMES, getGradientByIndex, getGradientName, CylinderMask, CylinderMaskInverted } from './Config.jsx';
...
const CONFIG = {
    ...
    CONTROL_BAR_PADDING_REM: 0.75,       // Padding horizontal du ControlBar (rem)
    CONTROL_BAR_GAP_REM: 0.75,           // Gap entre les éléments du ControlBar (rem)
...
const VibeCard = ({ vibeId, vibeName, availableCount, unavailableCount, isVibe, onClick, isExpired, colorIndex, onColorChange, onSwipeProgress, isBlinking, onBlinkComplete, onNameEdit, isEditingName, editedName, onEditedNameChange, onConfirmNameChange, onEditVibe, animationIndex = 0, animationKey = 0, animationDelay = 0, showTitles = true, is3DMode = false, swipeIndicator = null }) => {
    ...
    }, [colorIndex, vibeId, onSwipeProgress, isSwipeCatchingUp, is3DMode]);
    ...
    return (
        <div
            ...
            style={{
                ...
                padding: `0 ${CONFIG.CONTROL_BAR_PADDING_REM}rem`,
                gap: `${CONFIG.CONTROL_BAR_GAP_REM}rem`
            }}
        >
            <TimeCapsule
...
const SortButton = ({ icon: Icon, mode, currentMode, onClick, isFirst, isLast, isWide, hideGlow = false, glowOpacity = 1, transitionDuration = 0, retrigger = false, is3DMode = false }) => {
  ...
  // En 3D, extrémités rectangulaires avec coins 0.5rem au lieu de rounded-full
  let roundedClass = '';
  let extremityStyle = {};
  if (isFirst) {
      roundedClass = is3DMode ? '' : 'rounded-l-full';
      if (is3DMode) extremityStyle = { borderTopLeftRadius: '0.5rem', borderBottomLeftRadius: '0.5rem' };
  }
  if (isLast) {
      roundedClass = is3DMode ? '' : 'rounded-r-full';
      if (is3DMode) extremityStyle = { borderTopRightRadius: '0.5rem', borderBottomRightRadius: '0.5rem' };
  }
  ...
  return (
    <div className={`${widthClass} h-full relative overflow-visible ${roundedClass}`} style={extremityStyle}>
        ...
        {!isActive && (
            <CylinderMask intensity={CONFIG.CAPSULE_CYLINDER_INTENSITY_OFF} className={roundedClass} is3DMode={is3DMode} style={extremityStyle} />
        )}
        {isActive && (
            isRetriggering ? (
              <div
        ...
        {isActive && (
            <CylinderMask intensity={CONFIG.CAPSULE_CYLINDER_INTENSITY_ON} className={roundedClass} is3DMode={is3DMode} style={extremityStyle} />
        )}
        <button
            onClick={handleClick}
...
const ToggleSortButton = ({
    ...
    // En 3D, extrémités rectangulaires avec coins 0.5rem au lieu de rounded-full
    const roundedClass = isFirst ? (is3DMode ? '' : 'rounded-l-full') : '';
    const extremityStyle = isFirst && is3DMode ? { borderTopLeftRadius: '0.5rem', borderBottomLeftRadius: '0.5rem' } : {};
...
const TimeCapsule = ({ currentTime, duration, onSeek, onSkipBack, onSkipForward, isLive, isMini, confirmMode, confirmType, vibeSwipePreview, onScrubChange, onScrubEndAtEnd, is3DMode = false }) => {
    ...
    if (confirmMode) {
        ...
        return (
            <div
            className={`flex-1 ${is3DMode ? '' : 'rounded-full'} flex items-center justify-center border animate-appear overflow-hidden relative ${animClass}`}
                style={{
                    height: UNIFIED_CONFIG.FOOTER_BTN_HEIGHT,
                    borderRadius: is3DMode ? '0.5rem' : undefined,
...
const FeedbackOverlay = ({ feedback, onAnimationComplete, neonColor, bgClass, bgStyle, borderClass, children, roundedClass = 'rounded-full', is3DMode = false }) => {
    ...
    const effectiveRoundedClass = is3DMode ? '' : roundedClass;
  ...
  const effectivePortalRef = portalTarget || containerRef;
    ...
    return (
        ...
        {(() => {
          ...
          // Coordonnées du point indicateur
          let dotX, dotY;
          if (is3DMode) {
            // 3D: Le point parcourt uniquement le bord VERTICAL droit (exclut les coins arrondis 0.5rem = 8px)
            const cornerRadius = 8; // 0.5rem
            const verticalRange = beaconHeightPx - cornerRadius * 2;
            dotX = 0; // Collé au bord droit
            dotY = -verticalRange / 2 + progress * verticalRange;
          } else {
            // 2D: Le point parcourt le demi-cercle DROIT (de -90° à +90°)
            const angle = -Math.PI / 2 + progress * Math.PI;
            dotX = borderRadius * Math.cos(angle);
            dotY = borderRadius * Math.sin(angle);
          }
            ...
            <>
              ...
              {totalSongs > 0 && (
                <div
                  ...
                  style={{
                    right: is3DMode
                      ? `calc(${horizontalMarginPercent}% - ${CONFIG.BEACON_PULSE_SIZE / 2}px)`
                      : `calc(${horizontalMarginPercent}% + ${borderRadius - dotX - CONFIG.BEACON_PULSE_SIZE / 2}px)`,
              ...
              {/* Zone de touch pour déclencher le scrubbing */}
              {canScrub && (
                <div
                  ...
                  style={{
                    ...
                    top: '50%',
                    transform: 'translateY(-50%)',
                    width: is3DMode ? 40 : borderRadius + 20,
                    height: beaconHeightPx + 20,
                    borderRadius: is3DMode ? '0 0.5rem 0.5rem 0' : `0 ${borderRadius + 10}px ${borderRadius + 10}px 0`,
                  }}
        ...
        {isScrubbing && ReactDOM.createPortal((() => {
          ...
          if (is3DMode) {
            ...
            // Valeurs interpolées
            const barHeight = initialBarHeight + (finalBarHeight - initialBarHeight) * morphProgress;
            const barWidth = initialBarWidth + (finalBarWidth - initialBarWidth) * morphProgress;
            ...
            // Positions linéaires verticales
            const barTop = centerY - barHeight / 2;
                ...
                >
                  ...
                  <CylinderMask
                    ...
                    style={{
                      ...
                      height: barWidth,
                      position: 'absolute',
                      left: (barWidth - barHeight) / 2,
                      top: (barHeight - barWidth) / 2,
                    }}
                  />
                </div>
            ...
            // Valeurs interpolées
            const arcRadius = initialArcRadius + (finalArcRadius - initialArcRadius) * morphProgress;
            const centerX = initialCenterX + (finalCenterX - initialCenterX) * morphProgress;
            const centerY = initialCenterY + (finalCenterY - initialCenterY) * morphProgress;
            const thickness = initialThickness + (finalThickness - initialThickness) * morphProgress;
...
const handlePlayerTouchEnd = () => {
    ...
    // ===== LIBRARY HELPERS =====
    // Récupérer un morceau complet depuis la library (avec file du cache)
    const getSongFromLibrary = useCallback((songId) => {
        const song = library[songId];
        if (!song) return null;
        return {
            ...song,
            file: fileCache[songId] || null
        };
    }, [library, fileCache]);

    // Récupérer plusieurs morceaux depuis la library
    const getSongsFromLibrary = useCallback((songIds) => {
        if (!songIds) return [];
        return songIds.map(id => getSongFromLibrary(id)).filter(Boolean);
    }, [getSongFromLibrary]);

    // Récupérer les morceaux d'une vibe
    const getVibeSongs = useCallback((vibeId) => {
        const vibe = playlists?.[vibeId];
        if (!vibe) return [];
        // Support ancien format (songs array) et nouveau format (songIds array)
        if (vibe.songs) {
    ...
    // === FORMAT BIBLIOTHÈQUE - toutes les songs depuis la library ===
    const allGlobalSongs = React.useMemo(() => {
      return Object.values(library).map(song => ({
        ...song,
        file: fileCache[song.id] || null
      }));
    }, [library, fileCache]);
...
useEffect(() => {
  ...
  sessionStorage.setItem('vibes_splash_played', 'true'); // Marquer comme joué (survit au rechargement de page)
    ...
    if (playlists !== null && Object.keys(playlists).length > 0) {
        ...
        Object.keys(playlists).forEach(vibeId => {
            ...
            playlistsToSave[vibeId] = {
                name: vibe.name,
                // Support les deux formats pendant la transition
                songIds: vibe.songIds || (vibe.songs ? vibe.songs.map(s => s.id) : [])
            };
        });
...
// 2b. SAUVEGARDE AUTOMATIQUE - LIBRARY (tous les morceaux)
useEffect(() => {
...
const vibeSearchResults = () => {
  ...
  const processFileImport = (foldersToImport) => {
    // === FORMAT BIBLIOTHÈQUE ===
    // foldersToImport: { nomDossier: [File objects] }
    ...
    // Map fileSignature -> songId pour déduplication
    const signatureToId = new Map();
    Object.values(newLibrary).forEach(song => {
        if (song.fileSignature) {
            signatureToId.set(song.fileSignature, song.id);
        }
    });

    // Signatures de vibes existantes pour éviter les doublons
    ...
    Object.values(newPlaylists).forEach(vibe => {
        ...
        if (ids.length > 0) {
            existingVibeSignatures.add([...ids].sort().join('|'));
        }
    });

    // Map pour regrouper les imports du même dossier
    ...
    Object.keys(foldersToImport).forEach(folderName => {
        ...
        const songIdsForVibe = [];

        filesInFolder.forEach(file => {
            const fileSignature = file.name;

            // Parser titre/artiste
            ...
            if (newLibrary[songId]) {
                // Song existe déjà - mettre à jour avec fichier local
                newLibrary[songId] = {
                    ...newLibrary[songId],
                ...
                newLibrary[songId] = {
                    ...
                    localFileName: file.name,
                    dropboxPath: null,
                    dropboxAvailable: undefined,
                    fileSignature
                };
            }

            // Stocker blob URL dans le cache
            newFileCache[songId] = URL.createObjectURL(file);
        ...
        if (vibeSignature && existingVibeSignatures.has(vibeSignature)) {
            return; // Skip, vibe identique existe déjà
        }
        existingVibeSignatures.add(vibeSignature);

        // Regrouper ou créer nouvelle vibe
        const existingImportVibeId = importNameToVibeIdMap.get(folderName);
        if (existingImportVibeId) {
            // Ajouter à vibe existante de cet import
            newPlaylists[existingImportVibeId] = {
                ...newPlaylists[existingImportVibeId],
                songIds: [...(newPlaylists[existingImportVibeId].songIds || []), ...songIdsForVibe]
            };
        } else {
            // Créer nouvelle vibe
            const newVibeId = generateVibeId();
  ...
  const processDropboxImport = (foldersToImport) => {
    // === FORMAT BIBLIOTHÈQUE ===
    // foldersToImport: { nomDossier: [{ name, path_lower, ... }] }

    const newLibrary = { ...library };
    const newPlaylists = playlists ? { ...playlists } : {};
    const newVibeIds = [];

    // Fonction pour normaliser un titre (pour matching intelligent)
    ...
    Object.values(newLibrary).forEach(song => {
        ...
        const normalized = normalizeTitle(song.fileSignature || song.title || "");
        if (normalized && !titleToId.has(normalized)) {
            titleToId.set(normalized, song.id);
        }
    });
  ...
  // === FORMAT BIBLIOTHÈQUE - mettre à jour directement dans la library ===
  const updatePlayCount = (songToUpdate) => {
    if (!songToUpdate?.id) return;
    setLibrary(prev => ({
      ...prev,
      [songToUpdate.id]: {
        ...prev[songToUpdate.id],
        playCount: (prev[songToUpdate.id]?.playCount || 0) + 1
      }
    }));
  };
  ...
  const addToPlayNext = (song) => { 
    ...
    if (existingIdx !== -1) {
      ...
      // === FORMAT BIBLIOTHÈQUE - ajouter à la playlist active ===
      if (activeVibeId && playlists[activeVibeId]) {
        const vibe = playlists[activeVibeId];
        const songIds = vibe.songIds || [];
        const isInPlaylist = songIds.includes(song.id);
        if (!isInPlaylist) {
          // S'assurer que le song est dans la library
          if (!library[song.id]) {
            setLibrary(prev => ({
              ...prev,
              [song.id]: {
                ...
                dropboxPath: song.dropboxPath,
                dropboxAvailable: song.dropboxAvailable,
                fileSignature: song.fileSignature
              }
            }));
          }
          // Ajouter le songId à la vibe
          setPlaylists(prev => ({
            ...prev,
            [activeVibeId]: {
              ...prev[activeVibeId],
              songIds: [...(prev[activeVibeId].songIds || []), song.id]
            }
          }));
        }
      }
  ...
  const playFolder = (vibeId) => {
        const vibe = playlists[vibeId];
        const vibeSongs = getVibeSongs(vibeId);
        if (!vibe || vibeSongs.length === 0) {
    ...
    const doLaunchVibe = (vibeId) => {
      const vibe = playlists[vibeId];
    // Récupérer les songs depuis la library
    const vibeSongs = getVibeSongs(vibeId);
...
const cancelKillVibe = () => {
  ...
  // === FORMAT BIBLIOTHÈQUE - scan directement la library ===
  const scanDropboxAvailability = async () => {
    ...
    // Collecter les dropboxPath directement depuis la library
    const pathToSongId = new Map(); // path -> songId
    Object.values(library).forEach(song => {
        if (song.dropboxPath) {
            pathToSongId.set(song.dropboxPath, song.id);
        }
    });

    setScanDebugInfo(prev => ({ ...prev, pathsCount: pathToSongId.size }));

    if (pathToSongId.size === 0) {
        setScanDebugInfo(prev => ({ ...prev, error: 'no paths' }));
        return;
    }

    const pathsArray = Array.from(pathToSongId.keys());
    ...
    setLibrary(prev => {
        const updated = { ...prev };
        pathToSongId.forEach((songId, path) => {
            if (updated[songId]) {
                updated[songId] = {
                    ...updated[songId],
                    dropboxAvailable: !unavailablePaths.has(path)
                };
            }
        });
        return updated;
  ...
  // Lancer le scan au démarrage si connecté à Dropbox (après que library soit chargée)
  const scanTriggeredRef = useRef(false);
  useEffect(() => {
    const hasToken = !!getRefreshToken();
    const libraryLoaded = Object.keys(library).length > 0;
    setScanDebugInfo(prev => ({ ...prev, hasRefreshToken: hasToken, libraryLoaded }));

    // Attendre que library soit chargée et qu'on n'ait pas déjà lancé le scan
    if (!hasToken || !libraryLoaded || scanTriggeredRef.current) return;
    ...
    return () => clearTimeout(timer);
  }, [library]); // Re-évaluer quand library change
...
const importDropboxFolder = async (folderPath, folderName) => {
  ...
  try {
      ...
      Object.values(newPlaylists).forEach(vibe => {
          const songIds = vibe.songIds || [];
          if (songIds.length > 0) {
              const signature = [...songIds].sort().join('|');
      ...
      mp3Files.forEach((file) => {
          ...
          if (newLibrary[songId]) {
              // Song existe - mettre à jour dropboxPath
              newLibrary[songId] = {
                  ...newLibrary[songId],
                  dropboxPath: file.path_lower,
                  dropboxAvailable: true
      ...
      const folderSignature = [...songIdsForVibe].sort().join('|');
      ...
      {!isOnRealDevice && (
        ...
        >
          ...
          {(isLibrarySearching || searchOverlayAnim !== 'none') ? (
                <div className="w-full relative">
                    <div
                        className={`w-full ${is3DMode ? '' : 'rounded-full'} ${is3DMode ? '' : 'border border-gray-100'} shadow-sm flex items-center overflow-visible`}
                        style={{
              ...
              <div ref={headerButtonsRef} className="relative w-full" style={{ height: CONFIG.HEADER_BUTTONS_HEIGHT }}>
                  ...
                  {dropboxLoading && !showDropboxBrowser && (
                      ...
                      >
                          <div
                              className={`w-full ${is3DMode ? '' : 'rounded-full'} flex items-center justify-center gap-2`}
                              style={{
                                  height: CONFIG.HEADER_BUTTONS_HEIGHT,
                                  borderRadius: is3DMode ? '0.5rem' : undefined,
                                  background: '#0061FE',
                  ...
                  {vibeSwipePreview && vibeSwipePreview.progress > 0 && !pendingVibe && !nukeConfirmMode && !showImportMenu && importOverlayAnim === 'none' && !isLibrarySearching && searchOverlayAnim === 'none' && (() => {
                      ...
                      return (
                          <div className="absolute inset-0 flex items-center z-50">
                              <div
                                  className={`w-full ${is3DMode ? '' : 'rounded-full'} flex items-center justify-center border-none shadow-lg relative overflow-hidden`}
                                  style={{
                                      height: CONFIG.HEADER_BUTTONS_HEIGHT,
                                      borderRadius: is3DMode ? '0.5rem' : undefined,
                                      background: gradientBg,
          ...
          {librarySearchQuery ? (
              ...
              <div className="flex flex-col" style={{ gap: `${CONFIG.VIBECARD_GAP_VH}vh`, marginTop: `${CONFIG.HEADER_CARDS_GAP}rem` }}>
                    {Object.keys(playlists).map((vibeId, index) => {
                        const vibe = playlists[vibeId];
                        // === FORMAT BIBLIOTHÈQUE - récupérer les songs depuis la library ===
                        const songs = getVibeSongs(vibeId);
        ...
        {(!showFullPlayer || isPlayerClosing) && currentSong && (
                ...
                >
                    ...
                    {isProgressScrubbing && (() => {
                        const paddingRem = CONFIG.CONTROL_BAR_PADDING_REM;
                        const gapRem = CONFIG.CONTROL_BAR_GAP_REM;
                        ...
                        // left = padding + bordure, width = écran - 2×padding - 2×gap - 2×bordure - recenter - playpause
                        const tcLeft = `calc(${paddingRem}rem + ${borderWidth}px)`;
                ...
                {isProgressScrubbing && (() => {
                    ...
                    const paddingPx = CONFIG.CONTROL_BAR_PADDING_REM * 16;
                    const gapPx = CONFIG.CONTROL_BAR_GAP_REM * 16;
                    ...
                    const finalBottomPx = safeAreaBottom + footerPaddingTopPx + height + CONFIG.SCRUB_OVERLAY_OFFSET_REM * 16;
                    const currentBottomPx = initialBottomPx + (finalBottomPx - initialBottomPx) * t;

                    // Left : du bord TimeCapsule vers 1rem
                    const initialLeftPx = paddingPx;
                    ...
                    const playPauseWidth = UNIFIED_CONFIG.FOOTER_BTN_HEIGHT;
                    const recenterWidth = UNIFIED_CONFIG.FOOTER_BTN_HEIGHT * 1.6;
                    const initialRightPx = paddingPx + gapPx * 2 + playPauseWidth + recenterWidth;
            ...
            >
                ...
                <LiveHeaderCapsule
                    ...
                    <button
                        ...
                        className={`flex-shrink-0 ${is3DMode ? '' : 'rounded-full'} flex items-center justify-center shadow-sm transition-colors ${is3DMode ? '' : 'border border-gray-100'} text-gray-400 hover:bg-gray-100 relative overflow-hidden`}
